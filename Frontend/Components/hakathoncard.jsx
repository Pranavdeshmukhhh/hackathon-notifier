...
function cleanLocation(raw) {
  if (!raw) return '';
  return raw.replace(/^(online|offline)[,\s]*/i, '').replace(/[,\s]*(online|offline)$/i, '').trim();
}
...
const HackathonCard = ({ hackathon }) => {
  ...
  return (
    <div className="hack-card" style={{ opacity: isPast ? 0.6 : 1 }}>
      <div className="hack-card-top-row">
        <span className="badge source-badge">{source}</span>
        {hackathon.status && (
          <span className="badge status-badge">{hackathon.status}</span>
        )}
        {isTopCollege && (
          <span className="badge college-badge" title={collegeName}>
            🏛 {collegeType}
          </span>
        )}
        {isInternship && <span className="badge">💼 Internship</span>}
      </div>
